...
You're strongly encouraged to read this introduction and [Getting started](getting-started.mdx) for some basic background and context, before you dive right into development. [Working with manifests](manifest/understanding.md) also has some useful information, regardless of which language and library you use.
...
The "cr" icon (or "cr" pin) is trademarked by the C2PA and is the _de facto_ mark for C2PA user experiences. [Conforming products](conformance.mdx) can use the "Content Credentials" branding and the "Cr" pin to provide a consistent user experience and set expectations that an application, tool, or website implements C2PA standards.
