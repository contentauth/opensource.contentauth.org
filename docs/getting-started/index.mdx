...
The _C2PA trust list_ is a curated list of certification authorities (CAs) that are authorized to issue signing certificates for Generator Products. Conforming validator products such as the [Inspect tool on Adobe Content Authenticity (Beta)](https://verify.contentauthenticity.org) use the C2PA trust list to determine whether a Content Credential was issued using a certificate that can be traced back to an authorized CA.

The interim trust list provided critical support during the early adoption phase of C2PA. The C2PA [Verify tool](https://verify.contentauthenticity.org) used it to determine which certificates were considered valid and to prevent unknown signers from appearing as valid. Verify will be updated soon to use the official [C2PA trust list](conformance.mdx#c2pa-trust-lists).
