...
Similarly, using the Rust SDK, you can [add a manifest to an asset file](https://docs.rs/c2pa/latest/c2pa/#adding-a-signed-manifest-to-a-file), referencing the certificate and private key file. The [Node.js](../sdk-repos/c2pa-js/packages/c2pa-node/), [Python](../sdk-repos/c2pa-python), and [C++](../sdk-repos/c2pa-cpp) libraries can also add and sign a manifest.
...
You now have all the needed information to configure C2PA Tool for manifest signing. Edit your [manifest store file](../sdk-repos/c2pa-rs/cli/docs/manifest.md) to add the following fields that are specific to C2PA Tool:
...
You can now use C2PA Tool [to add a manifest to an image or other asset file](../sdk-repos/c2pa-rs/cli/docs/usage.md#adding-a-manifest-to-an-asset-file). The command will be something like this:
