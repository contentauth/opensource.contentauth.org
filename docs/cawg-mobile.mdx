...
This tutorial is a high-level guide with sample code for implementing the [Creator Assertions Working Group (CAWG)](https://cawg.io/) specifications using the [C2PA SDK for Android](https://github.com/contentauth/c2pa-android) and the [C2PA SDK for Swift](https://github.com/contentauth/c2pa-swift).
...
**SDK version.** CAWG support requires at least **c2pa-android 0.0.10 c2pa-swift 0.0.13**. Earlier releases can support the use of CAWG metadata or training-mining assertions, but do not include `Signer.withCawgIdentity` for CAWG Identity assertion signing.

**Reference implementation.** Every snippet below is taken from working code in the [Proofmode for Android](https://github.com/guardianproject/proofmode-android/tree/main/android-libproofmode/src/main/java/org/witness/proofmode/c2pa) or [Proofmode for iOS](https://gitlab.com/guardianproject/proofmode/proofmode-ios) codebase, primarily:
...
- [`C2PAHelper.swift`](https://gitlab.com/guardianproject/proofmode/proofmode-ios/-/blob/main/Proofmode/Shared/C2PAHelper.swift): the same assertions, signer setup, and signing on iOS
...
**What you already need working.** This guide assumes you can already produce a signed C2PA manifest with the [C2PA Android SDK](/docs/sdk-repos/c2pa-android/) (a `Signer`, a manifest definition, and a successful `Builder.sign()` call) or similar with [C2PA Swift](/docs/sdk-repos/c2pa-swift/). CAWG is layered on top of that, not a replacement for it.
