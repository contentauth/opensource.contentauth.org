...
Once you hold both PEMs, the identity signer is an ordinary `Signer`. The argument order in both SDKs: **certificate chain first, then private key**.
...
For an independent check, inspect the signed file with [c2patool](https://github.com/contentauth/c2pa-rs/tree/main/cli) or inspect it wit [Adobe Content Authenticity (Beta)](https://inspect.cr/).
